...
export default function EditTeacherForm({ teacherId, profileId, defaults }: Props) {
  ...
  async function onSubmit(values: FormValues) {
    setServerError(null)
    const result = await updateTeacher(teacherId, profileId, { ...values, specialties })
    if (result.error) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-2xl">
      ...
      <div className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm p-6 space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <label htmlFor="active" className="text-sm text-[#374151]">Professor ativo</label>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm p-6 space-y-5">
        ...
        <div className="space-y-2">
          <label className="block text-xs font-medium text-[#374151]">Especialidades</label>
