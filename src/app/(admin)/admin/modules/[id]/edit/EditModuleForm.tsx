...
export default function EditModuleForm({ moduleId, defaults }: Props) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-2xl">
      ...
      <div className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm p-6 space-y-5">
        ...
        <Field label="Título" error={errors.title?.message}>
          <input {...register('title')} type="text" className={inputCls(!!errors.title)} />
        </Field>

        <Field label="Descrição curta" error={errors.description?.message}>
          <input {...register('description')} type="text" className={inputCls(!!errors.description)} />
        </Field>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <Field label="Posição na trilha (0 = primeiro)" error={errors.orderIndex?.message}>
            <input {...register('orderIndex', { valueAsNumber: true })} type="number" min={0} className={inputCls(!!errors.orderIndex)} />
          </Field>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <label htmlFor="active" className="text-sm text-[#374151]">Módulo ativo</label>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm p-6 space-y-5">
        ...
        <Field label="Conteúdo (Markdown suportado)" error={errors.content?.message}>
          <textarea {...register('content')} rows={12} className={inputCls(false) + ' resize-y font-mono text-xs leading-relaxed'} />
        </Field>
      </div>
